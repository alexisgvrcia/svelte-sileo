...
import { AUTO_COLLAPSE_DELAY, AUTO_EXPAND_DELAY, DEFAULT_TOAST_DURATION, EXIT_DURATION } from './constants';
...
export function setStorePosition(pos: SileoPosition) {
	storePosition = pos;
}

export function setStoreOptions(opts: Partial<SileoOptions> | undefined) {
	storeOptions = opts;
}
...
const dismissToast = (id: string) => {
	...
	setTimeout(() => toastStore.update((prev) => prev.filter((t) => t.id !== id)), EXIT_DURATION);
};
...
export const sileo = {
	...
	clear: (position?: SileoPosition) =>
		toastStore.update((prev) => (position ? prev.filter((t) => t.position !== position) : [])),
};
